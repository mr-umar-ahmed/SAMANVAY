...
/** Where a task / block / train opens for each portal. */
export function routeForTask(portal: PortalId, taskId: string): string {
  ...
  if (portal === 'control') return `/app/control/blocks?task=${taskId}`;
  if (portal === 'division') return `/app/division/programme?task=${taskId}`;
...
export function routeForBlock(portal: PortalId, blockId: string): string {
  ...
  if (portal === 'control') return `/app/control/blocks?block=${blockId}`;
  if (portal === 'division') return `/app/division/audit?block=${blockId}`;
  if (portal === 'field') return `/app/field?block=${blockId}`;
  return `/app/planning/weekly?block=${blockId}`;
}
export function routeForTrain(portal: PortalId, number: string): string {
  if (portal === 'control') return `/app/control/corridor?train=${number}`;
  if (portal === 'field') return `/app/field/train?train=${number}`;
  if (portal === 'citizen') return `/citizen/train/${number}`;
  return `/app/planning/capacity?train=${number}`;
}
    ...
    if (snapshot) {
      ...
      for (const st of snapshot.corridor.stations) {
        if (st.code.toLowerCase().includes(query) || st.name.toLowerCase().includes(query))
          out.push({ id: st.code, kind: 'station', title: `${st.code} · ${st.name}`, sub: `km ${st.km}${st.junction ? ' · junction' : ''}`, route: portal === 'citizen' ? `/citizen/station/${st.code}` : portal === 'control' ? `/app/control/corridor?station=${st.code}` : `/app/planning/capacity?station=${st.code}` });
      }
    }
